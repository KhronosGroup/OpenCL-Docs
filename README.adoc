...
formal OpenCL API, OpenCL Extensions, OpenCL SPIR-V Environment, and OpenCL
C++ Specifications.
...
OpenCL Working Group, in the [OpenCL-Docs Github
repository](https://github.com/KhronosGroup/OpenCL-Docs). The documents are
all in [Asciidoctor](https://asciidoctor.org/) format.
...
The targets are generated in `out/html/` for HTML targets, and `out/pdf/`
for PDF targets. If a different output directory is desired, set the
Makefile variable OUTDIR on the command line. For example,
...
See the "`Vulkan Documentation and Extensions`" document in the Khronos
registry at
https://www.khronos.org/registry/vulkan/specs/1.0/styleguide.html for more
details of supported LaTeX math constructs in our toolchain.
...
[OpenCL Registry](https://www.khronos.org/registry/OpenCL). These Specifications
are placed under the proprietary Khronos specification copyright and license.
