...
CL_MUTABLE_DISPATCH_ASSERTS_KHR                    0x12B7
...
| This is a bitfield and can be set to a combination of the following values:
  ...
  _ceil((global_work_size[i] - global_work_offset[i])/local_work_size[i])_ (if
  _global_work_offset_ is NULL, _global_work_offset[i]_ should be replaced with _0_).
  In case _local_work_size_ is NULL, the effect of this flag is undefined.
|====
...
* {CL_INVALID_OPERATION} if {CL_MUTABLE_DISPATCH_ASSERT_NO_ADDITIONAL_WORK_GROUPS_KHR} is specified, 
