...
value. This new structure type can then be passed as an array element to
{clUpdateMutableCommandsKHR} to perform the update.
...
_config_types_ An array of length _num_configs_ with the type of each config in
_configs_.
...
interpreted using _config_types_ at each index.
  ...
  kernel void vector_addition(global int* tile1, global int* tile2,
    ...
    for (size_t i = 0; i < iterations; i++) {
      ...
      if (i != 0) {
        ...
        void* configs[1] = {&dispatch_config};
