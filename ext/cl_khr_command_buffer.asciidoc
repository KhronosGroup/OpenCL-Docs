...
A command-buffer object should increment the reference count of OpenCL objects
such as queues, buffers, images, and kernels referenced in commands recorded to
the command-buffer. This enables correct behaviour of the command-buffer when the
objects it uses have been released. On destruction of the command-buffer it should
decrement these reference counts, allowing the referenced objects to be freed if
appropriate.
...
the reference counting behaviour of {clSetKernelArg}.
====
