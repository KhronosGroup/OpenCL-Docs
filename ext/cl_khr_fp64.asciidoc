...
If extension pragma is enabled the floating point literals without
a suffix are interpreted as double precision if pragma disable is used
they are interpreted as single precision.
...
{
#pragma OPENCL EXTENSION cl_khr_fp64 : enable
  if (cos(1.0)) ...; // calls overload with the double type.
#pragma OPENCL EXTENSION cl_khr_fp64 : disable
  if (cos(1.0)) ...; // calls overload with the float type.
}
----------
