...
MANCOPYRIGHT = $(CURDIR)/config/copyright-ccby.txt $(MANDIR)/footer.txt
...
MANTMP	     = $(LOGFILE) $(REFPATH)/tocbody $(REFPATH)/toc.html \
	       $(REFPATH)/rewritebody $(REFPATH)/.htaccess
...
$(REFPATH)/apispec.txt: $(SPECFILES) $(GENREF) $(SCRIPTS)/reflib.py $(GENERATED)/api.py
	...
	$(CP) -p $(MANDIR)/footer.txt $(MANDIR)/*.* $(MANDIR)/static/*.txt $(REFPATH)
