// Copyright 2025 The Khronos Group. This work is licensed under a
...
* Isaac George, Qualcomm Technologies Inc.
...
cl_int clEnqueueSVMMemcpyWithPropertiesKHR(
...
Accepted value for the _param_name_ parameter to {clGetDeviceInfo} to query the size and alignment for concurrent access to SVM allocations:
...
| {CL_DEVICE_SVM_CONCURRENT_ACCESS_ATOM_SIZE_KHR}
  | {size_t_TYPE}[]
      | Queries the size and alignment in bytes that bound concurrent access to SVM allocations.
        Returns an array of values, with one value for each SVM type.
        Each returned value must be zero or a power of two.

        If the device supports concurrent access for an SVM type, then the concurrent access atom size must be non-zero, indicating that concurrent access is supported.
        If the device does not support concurrent access for an SVM type, then the concurrent access atom size must be zero, indicating that concurrent access is not supported.
|====
...
| {CL_SVM_CAPABILITY_CONCURRENT_ACCESS_KHR}
  | This type of SVM supports concurrent access from the host and a device, or from multiple devices.
    Use {CL_DEVICE_SVM_CONCURRENT_ACCESS_ATOM_SIZE_KHR} to determine the supported granularity for concurrent access.
...
| {CL_SVM_CAPABILITY_INDIRECT_ACCESS_KHR}
  | This type of SVM supports a single kernel enable to indicate that the kernel may access any allocation of this type, rather than passing a list of indirectly accessed allocations to the kernel.
|====
...
include::{generated}/api/protos/clSVMAllocWithPropertiesKHR.txt[]
include::{generated}/api/version-notes/clSVMAllocWithPropertiesKHR.asciidoc[]
...
include::{generated}/api/protos/clSVMFreeWithPropertiesKHR.txt[]
include::{generated}/api/version-notes/clSVMFreeWithPropertiesKHR.asciidoc[]
...
Applications should not rely on {clSVMFree} or {clSVMFreeWithPropertiesKHR} for synchronization, nor assume that {clSVMFree} or {clSVMFreeWithPropertiesKHR} cannot cause deadlocks.
====
...
include::{generated}/api/protos/clGetSVMPointerInfoKHR.txt[]
include::{generated}/api/version-notes/clGetSVMPointerInfoKHR.asciidoc[]
...
_ptr_ need not be a value returned by {clSVMAlloc} or {clSVMAllocWithPropertiesKHR}, but the query may be faster if it is.
...
include::{generated}/api/protos/clGetSVMSuggestedTypeIndexKHR.txt[]
include::{generated}/api/version-notes/clGetSVMSuggestedTypeIndexKHR.asciidoc[]
...
The suggested SVM type may be {CL_UINT_MAX}, indicating that there is no SVM allocation type for the _context_ and devices in _device_list_ that support the _required_capabilities_ and _properties_.
...
===== Using Copying SVM with Properties

The function

include::{generated}/api/protos/clEnqueueSVMMemcpyWithPropertiesKHR.txt[]
include::{generated}/api/version-notes/clEnqueueSVMMemcpyWithPropertiesKHR.asciidoc[]

enqueues a comment to copy to or from an SVM allocation with optional properties.
  ...
  * _properties_ is an optional list of properties for the buffer object and their corresponding values.
    ...
    When no properties are specified, the behavior of {clEnqueueSVMMemcpyWithPropertiesKHR} is identical to {clEnqueueSVMMemcpy}.
    When no properties are specified, if _src_ptr_ or _dst_ptr_ is a pointer to an SVM allocation, and the SVM allocation type does not include the {CL_SVM_CAPABILITY_SINGLE_ADDRESS_SPACE_KHR} capability, then the behavior of the copy is undefined.
    The {cl_khr_unified_svm_EXT} extension does not define any optional SVM memory copy properties.

{clEnqueueSVMMemcpyWithPropertiesKHR} returns {CL_SUCCESS} if the function is executed
successfully.
Otherwise, it returns one of the following errors:

  * Any of the errors for {clEnqueueSVMMemcpy}.
  * {CL_INVALID_PROPERTY}
...
--
*UNRESOLVED*:
Added a device query for {CL_DEVICE_SVM_CONCURRENT_ACCESS_ATOM_SIZE_KHR}.

Still TODO:

* What should the return type be for this query?  Specifically, should it be a {size_t_TYPE} or a {cl_uint_TYPE}?
* Do we want to distinguish between host concurrent access and device concurrent access, even if the only query we define for now is for host concurrent access?
* Are we OK with a single query for all types of SVM, or do we need a separate query for each SVM type?
* Is this only for concurrent access without atomic access?
--

. What other SVM allocation properties should we support?
+
--
`RESOLVED`: We decided not to accept any {cl_mem_flags_TYPE} or {cl_svm_mem_flags_TYPE}, and added access properties instead.
--
...
* Can a device "memcpy" from another device's allocation?  (Recommendation: Yes, but finalize as part of multi-device support.)
* Can a device "memcpy" to another device's allocation?  (Recommendation: Yes, but finalize as part of multi-device support.)
* Can a device "memcpy" to or from a SVM allocation in another context?  (No, undefined behavior.)
* Can a device "memcpy" to arbitrary host memory?  (Yes, we already have tests.)
...
--
`RESOLVED`:
Support for cacheability controls will not be included in this extension but they can be supported in a layered extension, if desired.
In a layered extension, we recommend adding cacheability properties instead of cacheability capabilities to avoid an explosion of capability combinations.
...
This appears to be the specified behavior for the C `memcpy` and `memset` functions.
.. A `ptr` equal to `NULL` is valid if and only if `size` is equal to zero, otherwise it is an error.

Allowing `size` to be zero and `ptr` to be `NULL` provides the most flexibility for other language runtimes built on top of OpenCL and the additional testing is manageable.
...
There is currently no defined error behavior for `size` equal to zero, so this will not need to be explicitly relaxed in this extension, but it will need to be stated explicitly and tested.

We should carefully consider whether we need additional restrictions around mutating a memory fill or a copy in a command buffer from a nonzero size to a size of zero, or vice versa
This is not a problem today, but it could be a problem when we have the proposed `cl_khr_command_buffer_mutable_memory_commands` extension.
...
Suggest adding a device query, perhaps as a layered extension, and use it to determine the maximum supported alignment error code.
See internal merge request 198.
--
...
. Can a pointer to a device, host, or shared SVM allocation be used to create a {cl_mem_TYPE} buffer using {CL_MEM_COPY_HOST_PTR}?
+
--
*UNRESOLVED*:
...
. Can a pointer to a device, host, or shared SVM allocation be passed to API functions to read from or write to {cl_mem_TYPE} objects, such as {clEnqueueReadBuffer} or {clEnqueueWriteImage}?
+
--
*UNRESOLVED*:
...
. Can a pointer to a device, host, or shared USM allocation be passed as the `pattern` argument to API functions to fill a {cl_mem_TYPE}, SVM allocation, or USM allocation, such as {clEnqueueFillBuffer}?
+
--
*UNRESOLVED*:
...
. Should we support passing traditional {cl_mem_flags_TYPE} via the USM allocation properties?
+
--
`RESOLVED`: We decided not to accept any {cl_mem_flags_TYPE} or {cl_svm_mem_flags_TYPE}, and added access properties instead.
--
...
+
--
*UNRESOLVED*: This issue may be easier to resolve now that this is a "unified SVM" extension vs. a "USM" extension, but it will still need more thought.

One particular enhancement we may want to consider, though, is whether calling {clGetEventInfo} and passing {CL_EVENT_COMMAND_EXECUTION_STATUS} to query the event status is a synchronization point.
...
Do not include a "rect" memcpy in the initial version of this extension.
A "rect" memcpy can always be added with a layered extension later, if desired.

Note that standard SYCL does not include a "rect" memcpy, though the https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/supported/sycl_ext_oneapi_memcpy2d.asciidoc[sycl_ext_onapi_memcpy2d] extension does include 2D memory copies and memory fills.
--
...
* For `cl_intel_unified_shared_memory`, calling {clDeviceMemAllocINTEL}, etc. returns {CL_INVALID_BUFFER_SIZE} if the size to allocate is zero.
* For CUDA, calling *cuMemAlloc*, etc. returns an error if the size to allocate is zero.
...
. Does the {clGetSVMSuggestedTypeIndexKHR} query apply to _all_ of the devices in the device list or context, or to _any_ of the devices in the device list or context?
+
--
...
|0.9.0|2024-10-29|Ben Ashbaugh|Initial public revision.
|========================================
