...
/* Enum values 0x419F-0x419F are reserved for future queries. */
----
...
| `CL_MEM_ALLOC_BUFFER_LOCATION_INTEL`
  | cl_intel_mem_alloc_buffer_location
      | Allows user to explicitly specify where the device/usm/host memory should be allocated by passing in ID of the buffer location. For device allocation, ID is the index of the global memory type as specified in the board_spec.xml.

        Property can be defined by: `cl_mem_properties_intel property[3] = { CL_MEM_ALLOC_BUFFER_LOCATION_INTEL, 0, 0};`

        Then passed onto allocation function by: `void *test_ptr = clDeviceMemAllocINTEL(m_context, m_device[0], &(property[0]), 8, alignment, &status);`

        Returns the same value as the ones previously passed onto `clDeviceMemAllocINTEL`, `clSharedMemAllocINTEL`, `clHostMemAllocINTEL`. If no buffer location specified, return ID of default buffer location `0` or the ID corresponding to `global_mem` with `default` property set in board_spec.xml.

|====

==== Specifying Which Global Memory Type To Allocate Device Memory

On FPGA, memory can be physically disjoint. A user might have DDRs, QDRs, SDRs plugged into the board. The different global memory type are specified in board_spec.xml. For example
