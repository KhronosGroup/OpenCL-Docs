...
In this table, `size_t` and `ptrdiff_t` are used as generic types to represent:
...
4+| For devices supporting OpenCL 3.1 or newer:
...
The following additional restrictions also apply to the *printf* instruction in
the *OpenCL.std* extended instruction set for OpenCL:

* Behavior is undefined for the asterisk *** field width modifier.
* Behavior is undefined when the *l* length modifier is followed by the *c*
  conversion specifier or the *s* conversion specifier.
* Behavior is undefined for the *ll*, *j*, and *L* length modifiers.
  Prior to OpenCL 3.1, behavior is additionally undefined for the *z* and *t*
  length modifiers.
* Behavior is undefined for the *n* conversion specifier.
* Behavior is undefined for the *l* length modifier when 64-bit integers are not
  supported.
* Behavior is undefined for the conversion specifier *s* when the operand is not
  a literal string.
