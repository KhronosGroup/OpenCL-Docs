...
Other changes in OpenCL 3.1:
  ...
  * Allowed optional image operands for image write instructions, see {khronos-opencl-pr}/1451[#1451].
  * Fixed a typo in the Image Data Types table header row, see {khronos-opencl-pr}/1531[#1531].
...
There were no changes from *v3.1.0* to *v3.1.1*.

Changes from *v3.1.1* to *v3.1.2*:

  * Documented more edge-case behavior directly vs. referencing the C99 specification, see {khronos-opencl-pr}/1539[#1539].
  * Fixed the image addressing algorithm rendering, see {khronos-opencl-pr}/1570[#1570].
  * Added a missing restriction when reading from an integer image with a sampler, see {khronos-opencl-pr}/1594[#1594].
