...
Changes from *v3.0.13*:
  ...
  * Corrected the precision for `cross` and `dot` to be based on `HALF_EPSILON` in `cl_khr_fp16`.
  * Added a context query for command-buffers to `cl_khr_command_buffer`.
  * Updated the semaphore wait and signal rules in `cl_khr_semaphore`.
  * Removed redundant error conditions from `cl_khr_external_semaphore` and `cl_khr_external_memory`.
  * Added new extension:
