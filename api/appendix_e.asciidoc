...
Changes from *v3.1.1* to *v3.1.2*:
  ...
  * Improved error code documentation consistency for memory object APIs, see {khronos-opencl-pr}/1589[#1589].
  * Clarified the behavior of {clSetProgramSpecializationConstant}, especially for separate compilation and linking, see {khronos-opencl-pr}/1602[#1602].
  * Clarified usage of the {CL_MEM_KERNEL_READ_AND_WRITE} memory flag, see {khronos-opencl-pr}/1605[#1605].
  * Clarified the behavior of {clGetSupportedImageFormats} for devices that do not support images, see {khronos-opencl-pr}/1606[#1606].
  * Clarified what is meant by a null string returned by {CL_PROGRAM_SOURCE}, see {khronos-opencl-pr}/1611[#1611].
  * Clarified when a device is considered to support non-uniform work-groups, see {khronos-opencl-pr}/1620[#1620].
  * {cl_khr_command_buffer_EXT}:
      ** Added a missing error condition for {clEnqueueCommandBufferKHR}, see {khronos-opencl-pr}/1588[#1588].
  * The following extensions has been finalized and is no longer experimental:
      ** {cl_khr_command_buffer_EXT}
