...
| {CL_DEVICE_EXTERNAL_MEMORY_IMPORT_ASSUME_DRIVER_HANDLE_TYPES_KHR_anchor}

include::{generated}/api/version-notes/CL_DEVICE_EXTERNAL_MEMORY_IMPORT_ASSUME_DRIVER_HANDLE_TYPES_KHR.asciidoc[]

  | {cl_external_memory_handle_type_khr_TYPE}[]
      | Returns the list of importable external memory handle types
        supported by _device_, that are assumed to be from a compatible
        external API.  When importing images, OpenCL may make assumptions
        about the underlaying memory layout, which may not be linear.

        When importing an image with clCreateImageWithProperties, if row_pitch or slice_pitch
        are non-zero, a linear row-major order will be assumed.
        ...
        {CL_DEVICE_EXTERNAL_MEMORY_IMPORT_ASSUME_DRIVER_HANDLE_TYPES_KHR}
        Handles types not in this list are assumed to be linear in row-major order.
endif::cl_khr_external_memory[]
