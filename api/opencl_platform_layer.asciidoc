...
returned in _errcode_ret_:
  ...
  * {CL_INVALID_PLATFORM} if _properties_ is `NULL` and no platform could be
    selected, or if the platform value specified in _properties_ is not a
    valid platform.
...
ifdef::cl_khr_gl_sharing[]
Addtionally, the following errors may be returned if the
`<<cl_khr_gl_sharing>>` extension is supported:

  * {CL_INVALID_GL_SHAREGROUP_REFERENCE_KHR} if a context
