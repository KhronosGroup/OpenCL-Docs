...
include::{generated}/api/version-notes/CL_DEVICE_EXTERNAL_MEMORY_IMPORT_ASSUME_VENDOR_OPTIMAL_IMAGES_HANDLE_TYPES_KHR.asciidoc[]

  | {cl_external_memory_handle_type_khr_TYPE}[]
      | Returns the list of importable external memory handle types supported 
        by _device_, that are assumed to have been exported by an external  API
        implementation from the same vendor, and has images that may be tiled  
        in a vendor-specific optimal layout.  If the UUID returned by CL_UUID 
        matches the device UUID of the external API  implementation that exported
        the handle being imported, the image layout is assumed to be accurately inferred.

        This list contains a subset of
        {CL_DEVICE_EXTERNAL_MEMORY_IMPORT_HANDLE_TYPES_KHR}.
        The returned list may be empty.

        If a handle type is in both {CL_DEVICE_EXTERNAL_MEMORY_IMPORT_ASSUME_LINEAR_IMAGES_HANDLE_TYPES_KHR} and {CL_DEVICE_EXTERNAL_MEMORY_IMPORT_ASSUME_VENDOR_OPTIMAL_IMAGES_HANDLE_TYPES_KHR}, 
        {CL_DEVICE_EXTERNAL_MEMORY_IMPORT_ASSUME_VENDOR_OPTIMAL_IMAGES_HANDLE_TYPES_KHR} will override the assume linear behavior.
