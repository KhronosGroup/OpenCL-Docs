...
include::{generated}/api/version-notes/CL_DEVICE_TYPE_CUSTOM.asciidoc[]
  | Specialized devices that implement some of the OpenCL runtime APIs but
    ...
    If an OpenCL device reports {CL_DEVICE_TYPE_CUSTOM} in its {CL_DEVICE_TYPE}
    then it cannot be combined with any other device types.
...
include::{generated}/api/version-notes/CL_DEVICE_TYPE_DEFAULT.asciidoc[]
  | The default OpenCL device in the platform.
    ...
    device when passed as the _device_type_ to {clGetDeviceIDs}, however
    {CL_DEVICE_TYPE_DEFAULT} must not be reported in {CL_DEVICE_TYPE} for any
    OpenCL device.
...
include::{generated}/api/version-notes/CL_DEVICE_TYPE_ALL.asciidoc[]
  | All OpenCL devices available in the platform, except for
    ...
    Only used to query OpenCL devices using {clGetDeviceIDs} and will never be
    returned as the {CL_DEVICE_TYPE} for any OpenCL device.
...
[NOTE]
====
Although the standard allows an OpenCL device to report more than one device
type in its {CL_DEVICE_TYPE}, many applications assume that devices will only
report a single device type. Implementations are strongly encouraged to report a
single device type for {CL_DEVICE_TYPE} and not to report a combination of
device types.
====
