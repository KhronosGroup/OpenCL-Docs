...
include::{generated}/api/protos/clCreateFromGLTexture.txt[]
  ...
  * _flags_ is a bit-field that is used to specify usage information.
    Refer to _table 5.3_ for a description of _flags_.
    Only {CL_MEM_READ_ONLY}, {CL_MEM_WRITE_ONLY} and {CL_MEM_READ_WRITE}
    values specified in _table 5.3_ may be used.
  * _texture_target_ must be one of `GL_TEXTURE_1D`, `GL_TEXTURE_1D_ARRAY`,
    ...
    `GL_TEXTURE_CUBE_MAP_NEGATIVE_Z`, or `GL_TEXTURE_RECTANGLE` (Note:
  ...
  * _texture_ is the name of a GL 1D, 2D, 3D, 1D array, 2D array, cubemap,
    ...
    image channel order and data type specified in _tables 5.5_ and _5.6_
...
returned in _errcode_ret_:
  ...
  * {CL_INVALID_MIP_LEVEL} if _miplevel_ is greather than zero and the
    ...
    levels.
  * {CL_INVALID_GL_OBJECT} if _texture_ is not a GL texture object whose
    type matches _texture_target_, if the specified _miplevel_ of _texture_
    is not defined, or if the width or height of the specified _miplevel_ is
  ...
  * {CL_INVALID_OPERATION} if _texture_ is a GL texture object created with
    a border width value greater than zero.
  * {CL_OUT_OF_RESOURCES} if there is a failure to allocate resources
    required by the OpenCL implementation on the device.
  * {CL_OUT_OF_HOST_MEMORY} if there is a failure to allocate resources
    required by the OpenCL implementation on the host.
...
include::{generated}/api/protos/clCreateFromGLRenderbuffer.txt[]
  ...
  * _renderbuffer_ is the name of a GL renderbuffer object.
    ...
    Only GL renderbuffers with internal formats that maps to appropriate
    image channel order and data type specified in _tables 5.5_ and _5.6_
...
returned in _errcode_ret_:
  ...
  * {CL_INVALID_IMAGE_FORMAT_DESCRIPTOR} if the OpenGL renderbuffer internal
    format does not map to a supported OpenCL image format.
  * {CL_INVALID_OPERATION} if _renderbuffer_ is a multi-sample GL
    renderbuffer object.
  * {CL_OUT_OF_RESOURCES} if there is a failure to allocate resources
    required by the OpenCL implementation on the device.
  * {CL_OUT_OF_HOST_MEMORY} if there is a failure to allocate resources
...
include::{generated}/api/protos/clGetGLObjectInfo.txt[]
  ...
  * _gl_object_type_ returns the type of GL object attached to _memobj_ and
...
[open,refpage='clGetGLTextureInfo',desc='Query additional information about the GL texture object associated with an OpenCL memory object',type='protos']
--
To query additional information about the GL texture object associated with
an OpenCL memory object, call the function
...
include::{generated}/api/protos/clEnqueueAcquireGLObjects.txt[]
  ...
  * _num_objects_ is the number of memory objects to be acquired in
    _mem_objects_.
  * _mem_objects_ is a pointer to a list of CL memory objects that
    correspond to GL objects.
...
to query or wait for this command to complete.
If _event_ is `NULL` or the enqueue is unsuccessful, no event will be
created and therefore it will not be possible to query the status of this
command or to wait for this command to complete.
If _event_wait_list_ and _event_ are not `NULL`, _event_ must not refer to
an element of the _event_wait_list_ array.
