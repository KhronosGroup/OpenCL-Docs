...
`cl_khr_command_buffer_multi_device` enables users to record commands across
multiple queues in the same command-buffer, providing execution of
heterogeneous task graphs from command-queues associated with different
devices.
...
Requires `<<cl_khr_command_buffer>>` version 0.9.3.
*Contact*::
  * TBD
