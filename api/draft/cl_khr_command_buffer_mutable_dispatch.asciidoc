...
`cl_khr_command_buffer_mutable_dispatch` enables users to modify the
configuration of kernel execution commands between command-buffer enqueues.
...
Requires `<<cl_khr_command_buffer>>` version 0.9.0.
*Contact*::
  * TBD
...
`cl_khr_command_buffer_mutable_dispatch` removes this restriction, in
particular, this extension allows the configuration of a kernel execution
...
Enums for working with mutable-command objects from <<mutable-commands,
Section 5.X.5>>:
...
+
--
*RESOLVED*: Can be included in the final release of the extension if there
is implementation coverage.
--
