...
The OpenCL memory model tells programmers what they can expect from an
...
The memory consistency model in OpenCL is based on the memory model from the
...
The OpenCL 2.0 specification defines a number of _synchronization
operations_ that are used to define memory order constraints in a program.
...
An OpenCL implementation shall ensure that no program execution demonstrates
...
This section discusses ordering rules for OpenCL 2.0 atomic operations.
...
As long as the following conditions are met, a host program sharing SVM
...
the kernel, are visible to each other:
  ...
  . The optional OpenCL 2.0 SVM atomic-controlled visibility specified by
...
This section describes how the OpenCL 2.0 fence operations contribute to the
...
2.0; _host command-queues_ and _device command-queues_.
...
If fine-grain SVM is used but without support for the OpenCL 2.0 atomic
...
The OpenCL 1.2 standard describes a synchronization point as a
