...
These prerequisites have three sources:
  ...
  * The third source of prerequisites can be the presence of non-trivial C
    ...
    In this case, the OpenCL C/{cpp} compiler must generate program
...
Each work-item's global ID is an N-dimensional tuple.
...
The global indices define a G~x~by G~y~ index space where the total number
...
The OpenCL execution model supports three types of kernels:
  ...
  * *Built-in kernels* are tied to a particular device and are not built at
...
{cl_khr_d3d10_sharing_EXT} extension is supported) or
{CL_CONTEXT_D3D11_DEVICE_KHR} (for Direct3D 11, if the
{cl_khr_d3d11_sharing_EXT} extension is supported.
...
Two actions *A* and *B* are defined to have an inclusive scope if:

  * *A* and *B* are executed by work-items within the same sub-group and the
    memory scope of *A* and *B* is *memory_scope_sub_group* or wider.
  * *A* and *B* are executed by work-items within the same work-group and the
    memory scope of *A* and *B* is *memory_scope_work_group* or wider.
  * *A* and *B* are executed by work-items in the same kernel-instance or a
    child-kernel of the kernel-instance, and the memory scope of *A* and *B* is
    *memory_scope_device* or wider.
  * *A* and *B* are executed by work-items on the same device, *A* and *B*
    operate on an SVM allocation, and the memory scope of *A* and *B* is
    *memory_scope_device* or wider.
  * *A* and *B* are executed by a host thread or by work-items on one or more
    devices that support SVM atomic operations, *A* and *B* operate on an SVM
    allocation, and the memory scope of *A* and *B* is
    *memory_scope_all_svm_devices* or *memory_scope_all_devices*.

Prior to OpenCL 3.1, the memory scope of *A* and *B* must additionally be the
...
following:
  ...
  . If *C* is a command that waits for an event *E*'s completion, and API
    function call *X* sets user event *E*'s status to
...
NOTE: Mixed version support is <<unified-spec, missing before>> version 1.1.
...
* An OpenCL 3.1 device must support all the requirements of an OpenCL 3.0 device in addition to the OpenCL C 3.1 programming language and the SPIR-V 1.0, SPIR-V 1.1, SPIR-V 1.2, SPIR-V 1.3, and SPIR-V 1.4 intermediate languages (ILs).
...
Applications can use these version macros along with the convenience macros
