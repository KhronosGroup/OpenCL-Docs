...
These prerequisites have three sources:
  ...
  ** Commands enqueued after a command-queue barrier that itself has no explicit
     prerequisites have all commands enqueued before the barrier as prerequisites.
  ** Commands enqueued in an in-order command-queue have the command enqueued before
