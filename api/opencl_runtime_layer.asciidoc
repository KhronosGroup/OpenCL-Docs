...
Otherwise, it returns one of the following errors:
  ...
  * {CL_INVALID_WORK_GROUP_SIZE} if _local_work_size_ is specified and is not
    ...
    program source.
  * {CL_INVALID_WORK_GROUP_SIZE} if _global_work_size_ is not a multiple of
    _local_work_size_ and _kernel_ is qualified with the
    `uniform_work_group_size` attribute in the program source.
    This error code is <<unified-spec, missing before>> version 3.0.
