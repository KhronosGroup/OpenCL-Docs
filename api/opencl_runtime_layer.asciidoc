...
==== Update to Command-Buffer Lifecycle

If this extention is supported, the command-buffer lifecycle is extended to
include a new state named **Finalized**.

// Image generated from the following mermaid diagram description using
// https://mermaid.live:
// ---
// config:
//  theme: default
...
//  if_state: All ND-range kernels commands' arguments set?
//  [*] --> Recording: Create
//  Recording --> if_state: Finalize
//  if_state --> Finalized: No
//  if_state --> Executable : Yes
//  Finalized --> if_state: Update mutable commands     
//  Executable --> Pending: Enqueue
//  Pending --> Executable: Completion

image::images/mutable_commandbuffer_lifecycle.png[align="center",
  title="Lifecycle of a mutable command-buffer."]
