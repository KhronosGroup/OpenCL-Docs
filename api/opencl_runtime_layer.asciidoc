...
or an image memory object from an external handle:
  ...
  * {CL_EXTERNAL_MEMORY_HANDLE_DMA_BUF_KHR_anchor} is a file descriptor for a Linux
    dma_buf. Once a memory object created from dma_buf has been acquired with 
    ...
    The implementation may make system calls to manage data coherency. Any operations on 
    the dma_buf while it is acquired may result in undefined behavior.
