...
Also see `<<cl_khr_create_command_queue>>`.
...
If an OpenGL texture object with an internal format from the table below is
...
[[mapping-of-gl-image-formats-table]]
...
=== Creating OpenCL Image Objects From OpenGL Renderbuffers
...
The <<mapping-of-gl-image-formats-table, OpenGL internal formats and
...
=== Synchronizing Access to Shared OpenGL/OpenCL Memory Objects

In order to ensure data integrity, the application is responsible for
synchronizing access to shared OpenCL/OpenGL objects by their respective
APIs.
...
subsequent OpenGL commands which reference these objects.
...
operations if the OpenCL and OpenGL contexts are in separate threads.
...
[[event-command-types-for-shared-opencl-opencl-memory-objects]]
=== Event Command Types for Shared OpenCL/OpenGL Memory Objects
...
ifdef::cl_khr_il_program[{clCreateProgramWithILKHR},]
    {clCreateProgramWithSource} or {clCreateProgramWithIL}.
  * {CL_OUT_OF_RESOURCES} if there is a failure to allocate resources required
    by the OpenCL implementation on the device.
  * {CL_OUT_OF_HOST_MEMORY} if there is a failure to allocate resources
    required by the OpenCL implementation on the host.
...
ifdef::cl_khr_gl_msaa_sharing[]
  * {CL_INVALID_MEM_OBJECT} for an argument declared to be a multi-sample
    image, multi-sample image array, multi-sample depth image or a
    multi-sample depth image array when the specified _arg_value_ does not
    follow the rules described above for a depth memory object or memory
...
//ifdef::cl_khr_command_buffer[{clEnqueueCommandBufferKHR}]
...
with {clCreateEventFromGLsyncKHR} can also be used to reflect the status of
an OpenGL sync object.
...
include::{generated}/api/version-notes/clCreateCommandBufferKHR.asciidoc[]
  ...
  * _num_queues_ is the number of command-queues listed in _queues_.
    This extension only supports a single command-queue, so this **must** be
    one.
  * _queues_ is a pointer to a command-queue that the command-buffer
    commands will be recorded to.
    _queues_ must be a non-`NULL` value.
...
include::{generated}/api/version-notes/clCommandCopyBufferKHR.asciidoc[]
  ...
  * _command_queue_ specifies the command-queue the command will be recorded
    to.
    This parameter is unused, as only a single command-queue is supported,
    and **must** be `NULL`.
...
include::{generated}/api/version-notes/clCommandNDRangeKernelKHR.asciidoc[]
  ...
  * _properties_ specifies a list of properties for the kernel command and
    ...
    The list is terminated with 0.
    If no properties are required, _properties_ may be `NULL`.
    This extension does not define any properties.
...
Otherwise, it returns one of the following errors:
  ...
  * {CL_INVALID_GL_SHAREGROUP_REFERENCE_KHR} if a context was specified by
    any of the following means:
+
--
* A context was specified for an EGL-based OpenGL ES or OpenGL
  implementation by setting the properties {CL_GL_CONTEXT_KHR} and
  {CL_EGL_DISPLAY_KHR}.
* A context was specified for a GLX-based OpenGL implementation by setting
  the properties {CL_GL_CONTEXT_KHR} and {CL_GLX_DISPLAY_KHR}.
* A context was specified for a WGL-based OpenGL implementation by setting
  the properties {CL_GL_CONTEXT_KHR} and {CL_WGL_HDC_KHR}.
--
+
