...
ifdef::cl_khr_external_memory_android_hardware_buffer[]
  * {CL_INVALID_BUFFER_SIZE} 
    ** if _size_ is non-zero and greater than the AHardwareBuffer when importing external memory 
    using {cl_khr_external_memory_android_hardware_buffer_EXT}
  * {CL_INVALID_OPERATION}
    ** if the AHardwareBuffer format is not `AHARDWAREBUFFER_FORMAT_BLOB`
  * {CL_INVALID_HOST_PTR}
    ** if _host_ptr_ is not `NULL`.
endif::cl_khr_external_memory_android_hardware_buffer[]
ifdef::cl_ext_buffer_device_address[]
  * {CL_INVALID_OPERATION}
    ** of _properties_ includes {CL_MEM_DEVICE_PRIVATE_ADDRESS_EXT} and there
  are no devices in the context that support the {cl_ext_buffer_device_address_EXT}
  extension.
endif::cl_ext_buffer_device_address[]
ifdef::cl_ext_immutable_memory_objects[]
  * {CL_INVALID_VALUE}
    ** if {CL_MEM_IMMUTABLE_EXT} is set in _flags_ and
...
ifdef::cl_khr_external_memory_android_hardware_buffer[]
  ...
  ** if _image_format_ is not `NULL` when using {cl_khr_external_memory_android_hardware_buffer_EXT}.

  * {CL_INVALID_IMAGE_DESCRIPTOR}
  ** if _image_desc_ is not `NULL` when using {cl_khr_external_memory_android_hardware_buffer_EXT}.

  * {CL_IMAGE_FORMAT_NOT_SUPPORTED}
  ** if the AHardwareBuffer format is not supported.
