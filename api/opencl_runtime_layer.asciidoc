...
include::{generated}/api/version-notes/CL_IMAGE_ROW_PITCH.asciidoc[]
  | {size_t_TYPE}
      | Returns the row pictch in bytes of a row of elements of the
        image object given by _image_ when image data is linearly tiled.
...
include::{generated}/api/version-notes/CL_IMAGE_SLICE_PITCH.asciidoc[]
  | {size_t_TYPE}
      | Returns the slice pitch in bytes of a 2D slice for the 3D
        ...
        by _image_ when image data is linearly tiled. If _image_ was created
        with a non-zero value for `image_slice_pitch` then the value provided
        for `image_slice_pitch` by the application is returned, otherwise the
        returned value is calculated as: +
