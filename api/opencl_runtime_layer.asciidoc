...
{clFinalizeCommandBufferKHR}, but there is at least one ND-range kernel command
for which not all arguments have been set (this is a valid state only for
mutable command buffers).

[[executable]]
Executable:: State after command recording has finished with
{clFinalizeCommandBufferKHR} and all the kernel arguments of all ND-range kernel
...
//   [*] --> Recording:Create
//   Recording --> if_state:Finalize
//   if_state --> Finalized:Not all kernel arguments set
//   if_state --> Executable:All kernel arguments set
//   Finalized --> if_state:Update mutable commands
...
execution was successfully queued, or one of the errors below:
  ...
  * {CL_INVALID_COMMAND_BUFFER_KHR} if _command_buffer_ is not a valid
    command-buffer.
  * {CL_INVALID_OPERATION} if _command_buffer_ is not in the Executable state.
...
ifdef::cl_khr_command_buffer_mutable_dispatch[]
  ...
  * _kernel_ is a valid kernel object which **must** have its arguments set only
    if the command is immutable (see description of argument _mutable_handle_).
...
finalization (in which case, the command buffer is in Finalized state). Of
course, all the arguments must be set before the command buffer can be enqueued
