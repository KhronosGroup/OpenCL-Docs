...
ifdef::cl_khr_external_memory[]
    ...
    image row pitch.
  * _num_mip_levels_ and _num_samples_ must be 0.
...
Where a _compatible_ command-queue is defined as a command-queue with
...
used in a command has been modified or freed.
The only valid operation to perform on a command-buffer in the Invalid state
is to call {clReleaseCommandBufferKHR} for each of the reference counts the
application owns.
...
safe use of the command-buffer.
As the reference count of the kernel object will have been incremented when
the command was recorded, and then on command-buffer release the kernel
reference count will be decremented.
If at that point the kernel reference count reaches 0, the kernel object
will be freed.
====
...
include::{generated}/api/version-notes/clCommandBarrierWithWaitListKHR.asciidoc[]
  ...
  * _command_queue_ specifies the command-queue the command will be recorded
    to.
    This parameter is unused by `cl_khr_command_buffer`, as only a single
...
completed.
  ...
  * _mutable_handle_ returns a handle to the command.
    This parameter is unused by `cl_khr_command_buffer`, and **must** be
    `NULL`.
...
include::{generated}/api/version-notes/CL_COMMAND_BUFFER_STATE_PENDING_KHR.asciidoc[]
        ...
        {CL_COMMAND_BUFFER_STATE_INVALID_KHR_anchor} is returned when
        _command_buffer_ is in an <<invalid, Invalid>> state.
include::{generated}/api/version-notes/CL_COMMAND_BUFFER_STATE_INVALID_KHR.asciidoc[]
